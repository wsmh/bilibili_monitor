...
充电问答（upower）更新相对较慢，默认每 10 分钟扫描一次，可通过 `UPOWER_QA_SCAN_INTERVAL_SECONDS` 调整。首次启动默认会先做去重初始化而不推送历史回复（避免刷屏），如希望首次启动也推送最新回复，可设置 `UPOWER_QA_INIT_MODE=notify` 并用 `UPOWER_QA_INIT_NOTIFY_LIMIT` 控制推送条数。
